<?php $__env->startSection('title', '职员管理'); ?>
                ...
                <div class="ibox-content">
                    <div class="row">
                        <div class="col-sm-9 m-b-xs">
                            ...
                            <a class="btn btn-info btn-sm " type="button" href="/staff/create"><i class="glyphicon glyphicon-plus"></i> 新增</a>
                            <?php endif; ?>
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('delete staff')): ?>
                            <a class="btn btn-danger btn-sm " type="button" url="/staff/1" onclick="jumps($(this).attr('url'));"><i class="glyphicon glyphicon-trash"></i> 删除</a>
                            <?php endif; ?>
                            <a class="btn btn-warning btn-sm " type="button" href="/staff"><i class="glyphicon glyphicon-refresh"></i>  重置 / 刷新</a>
                        </div>

                        <div class="col-sm-3">
                            <form action="/staff" method="get">
                                <div class="input-group">
                                    <input type="text" placeholder="请按照职员编号，职员名称查询" class="input-sm form-control" name="search">
                        ...
                        <table class="table table-striped">
                            ...
                            <?php $__currentLoopData = $model; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    ...
                                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('status staff')): ?>
                                    <td class="text-center"><a url="/staff/<?php echo e($value->id); ?>/status" onclick="set_status($(this).attr('url'),this)"><?php echo $value->status; ?></a></td>
                                    <?php endif; ?>
                                    <td class="text-center">
                                        <a href="/staff/<?php echo e($value->id); ?>" class="btn btn-primary btn-sm " type="button"><i class="fa fa-money"></i> 查看</a>
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('edit staff')): ?>
                                        <a href="/staff/<?php echo e($value->id); ?>/edit" class="btn btn-success btn-sm " type="button"><i class="fa fa-paste"></i> 编辑</a>
                                        <?php endif; ?>
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('delete staff')): ?>
                                        <a class="btn btn-danger btn-sm " type="button" url="/staff/<?php echo e($value->id); ?>" onclick="jump($(this).attr('url'),<?php echo e($value->id); ?>);"><i class="glyphicon glyphicon-trash"></i> 删除</a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
