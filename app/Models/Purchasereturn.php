...
class Purchasereturn extends Model
{
    ...
    {
        $data['document_number'] = 'TH' . date('YmdHis');
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                DB::table('accounts')->where('id', $purchase->settlement_account)->increment('current_balance',$purchase->paid);
                $sum = 0;
                foreach ($dataExtend['data'] as $key => $value) {
                    $json_data = json_decode($value);
                    $purchasereturnExtend = PurchasereturnExtend::create([
                    ...
                    DB::table('goods')->where('id', $purchasereturnExtend->goods_id)->decrement('current_inventory',
                        $purchasereturnExtend->number);

                    DB::table('warehouse_goods_number')
                        ->where('warehouse_id', $purchasereturnExtend->warehouse_id)
                        ->where('goods_id', $purchasereturnExtend->goods_id)
                        ->decrement('number', $purchasereturnExtend->number);

                    $warehouseGoodsNumber = WarehouseGoodsNumber::where('warehouse_id', $purchasereturnExtend->warehouse_id)
                        ->where('goods_id', $purchasereturnExtend->goods_id)
                        ->get(['goods_id','number']);
                    $newArray = [];
                    ...
                    if ($newArray[0] ==0){
                        WarehouseGoodsNumber::where('warehouse_id', $purchasereturnExtend->warehouse_id)
                            ->where('goods_id', $purchasereturnExtend->goods_id)->delete();
                    }
                }
                ...
                DB::table('purchasereturns')->where('id', $purchase->id)->increment('total_amount',
                    $sum);
    ...
    //关联附表
    public function getPurchasereturnExtend()
    {
        return $this->hasMany(\App\Models\PurchasereturnExtend::class, 'purchase_id', 'id');
    }
