...
class AccountProceedsDetailNewController extends Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->middleware(['permission:index accountProceedsDetailNew']);
    }

    public function index(Request $request)
    {
        $client = Client::all();
        return view('accountProceedsDetailsNew.index', [
            'client' => $client,
    ...
    public function sqlData($getData)
    {
        ...
        $where = ['created_at', 'document_number', 'total_amount', 'paid', 'client_id'];
        ...
        $dataA = $this->arrayData($sales, '销货', ['已收完全款', '部分收款', '未收款']);

        $dataB = $this->arrayData($salesReturn,'销货退货',  ['已退完全款', '部分退款', '未退款']);
    ...
    {
        \Excel::create('应收账款明细表)' . date('YmdHis'), function ($excel) {
            $excel->sheet('应收账款明细表)', function ($sheet) {
