...
class PurchasereceiptsController extends Controller
{
    ...
    {
        parent::__construct();
        $this->middleware(['permission:index purchaseReceipts'])->only(['index']);
        $this->middleware(['permission:add purchaseReceipts'])->only(['create']);
    }
    ...
    public function index()
    {
        ...
        return view('purchaseReceipts.index', [
            'model' => $payment->allData(\request()->all(), 2),
    ...
    public function create()
    {
        ...
        $purchasesCount = DB::table('purchasereturns')->groupBy('supplier_id')->get(['supplier_id'])->count();
        return view('purchaseReceipts.create', compact('purchasesCount', 'settlement', 'account'));
    }
    ...
    public function show($id)
    {
        ...
        return view('purchaseReceipts.show', compact('payment'));
    }
    ...
    {
        $newArrayId = [];
        $ids = DB::table('purchasereturns')->groupBy('supplier_id')->get(['supplier_id']);
    ...
    public function get_purchase(Request $request)
    {
        if ($request->has('id')) {
            $num1 = $request->input('id');
            $num2 = ($num1 - 1) * 10;
            return Purchasereturn::where('supplier_id', $request->input('supplier_id'))
    ...
    public function get_details(Request $request)
    {
        if ($request->isMethod('post')) {
            $purchase = Purchasereturn::find($request->input('purchaseId'));
            $newArray = [];
            foreach ($purchase->getPurchasereturnExtend as $key => $value) {
    ...
    {
        \Excel::create('购货收款表' . date('YmdHis'), function ($excel) {
            $excel->sheet('购货收款表', function ($sheet) {
                $url = url()->previous();
