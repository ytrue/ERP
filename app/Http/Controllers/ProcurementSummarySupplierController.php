...
class ProcurementSummarySupplierController extends Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->middleware(['permission:index procurementSummarySupplier']);
    }
    ...
    public function index(Request $request)
    {
        ...
        return view('procurementSummarySupplier.index', [
            'goods' => $goods,
            'warehouse' => $warehouse,
    ...
    {
        \Excel::create('采购总汇表(按供应商)' . date('YmdHis'), function ($excel) {
            $excel->sheet('采购总汇表(按供应商)', function ($sheet) {
                $procurementDetailsController = new ProcurementDetailsController();
                $url = url()->previous();
                $newDataA = [];
                ...
                foreach ($newData as $key => $value) {
                    $newDataA[$key]['supplierManagement'] = $value['supplierManagement'];
