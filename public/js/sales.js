$(function () {
    ...
    $("#add_row").on('click', '.add_shop', function () {
        ...
        var xsry_name = $("#xsry_name").attr('staff_id');
        if (gys_name) {
            if (xsry_name) {
                if (warehouse_id) {
                ...
                swal({title: "error", text: "请先选择销售人员", type: "error"})
            }
    ...
    $("#add-ok").on('click', '.documentary', function () {
        ...
        for (var j = 0; j < $(".goods_name").length; j++) {
            if (i == $(".goods_name").eq(j).next().attr('test')) {
                ...
                $(".goods_name").eq(j).parents(":eq(3)").nextAll(":eq(2)").children().children().children().children().val($(this).parents(":eq(0)").children(":eq(3)").html());

                $(".goods_name").eq(j).parents(":eq(3)").nextAll(":eq(1)").children().children().children().children().val(1);
                $(".goods_name").eq(j).parents(":eq(3)").nextAll(":eq(3)").children().children().children().children().val(0);
                $(".goods_name").eq(j).parents(":eq(3)").nextAll(":eq(4)").children().html(0);
                $(".goods_name").eq(j).parents(":eq(3)").nextAll(":eq(5)").children().html($(this).parents(":eq(0)").children(":eq(3)").html());
    ...
    $("#add_row").on('click', '.add_warehouse', function () {
        ...
        if (gys_name) {
            ...
            swal({title: "error", text: "请先选择客户", type: "error"})
        }
    });
    ...
    $("#add_purchase_data").click(function () {
        ...
        for (var i = 0; i < $(".goods_name").length; i++) {
            dataExtend[i] = '{"goods_name":"' + $(".goods_name").eq(i).val() + '","goods_id":"' + $(".goods_name").eq(i).attr('goods_id') + '","warehouse_id":"' + $(".warehouse_name").eq(i).attr('warehouse_id') + '","number":"' + $('.number').eq(i).val() + '","company":"' + $(".company").eq(i).text() + '","unit_purchase_price":"' + $(".unit_purchase_price").eq(i).val() + '","discount_rate":"' + $(".discount_rate").eq(i).val() + '","purchase_amount":"' + $(".purchase_amount").eq(i).text() + '"}';
        }
        $.ajax({
            url: "/sales",
            type: 'post',
